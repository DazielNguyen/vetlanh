...
function SpotlightBentoCell({
  ...
  const bgImage = useMotionTemplate`radial-gradient(280px circle at ${mouseX}px ${mouseY}px, rgba(255, 138, 101, 0.10), transparent)`;
      ...
      <Card className="border-none card-lifted rounded-3xl overflow-hidden h-full">
        <CardHeader className="pb-2">
          ...
          <div className="flex items-baseline gap-2 mt-1">
            <span className="text-sm font-medium text-foreground/50">Căng thẳng</span>
            <span className="text-3xl font-baloo font-bold text-foreground">
              {dashboard?.stress_level
                ? STRESS_LEVEL_LABEL[dashboard.stress_level]
                : "Chưa có dữ liệu"}
            </span>
            <span className="text-sm text-emerald-700 dark:text-emerald-400 font-medium">
              {dashboard?.stress_trend_text ?? "Check-in để bắt đầu theo dõi"}
            </span>
          </div>
        </CardHeader>
